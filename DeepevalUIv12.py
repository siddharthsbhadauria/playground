...
def display_test_case_data(test_case):
    ...
    for item in test_case_tree.get_children():
        test_case_tree.delete(item)
...
def display_metrics_data(metrics):
    ...
    for metric in metrics:
        for key, value in metric.items():
            if isinstance(value, list):  # Convert list to string for display
                value = ", ".join(map(str, value))
            metrics_tree.insert('', 'end', values=(key, value))
...
# Bottom Frame: Data display (split into two sections)
...
metrics_label = tk.Label(bottom_frame, text="Metrics Information")
metrics_label.grid(row=2, column=0, padx=5, pady=5, sticky='w')

metrics_tree = ttk.Treeview(bottom_frame, columns=('Key', 'Value'), show='headings')
metrics_tree.heading('#1', text='Key')
metrics_tree.heading('#2', text='Value')
metrics_tree.column('#1', width=200, anchor='w')
metrics_tree.column('#2', width=600, anchor='w')
metrics_tree.grid(row=3, column=0, padx=5, pady=5, sticky='ew')

# Scrollbars for the trees
...
metrics_scrollbar = Scrollbar(bottom_frame, orient="vertical", command=metrics_tree.yview)
metrics_scrollbar.grid(row=3, column=1, sticky='ns')
